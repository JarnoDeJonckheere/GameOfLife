...
int main() {
    ...
    while(generation<max_generations){
        ...
        gameOfLifeKernel<<<gridDim, blockDim>>>(d_current, d_next, WIDTH, HEIGHT, generation); //updates to the next generation
        cudaDeviceSynchronize();
        // this can be done asynchronous?

        //copy this generations output to CPU
        cudaMemcpy(h_current, d_next,size,cudaMemcpyDeviceToHost);
          //write to the file
        for(int i =0;i<WIDTH*HEIGHT;i++){
            outPutFile<<h_current[i]<<";";
        }
