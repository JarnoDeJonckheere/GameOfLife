...
__global__ void gameOfLifeKernel(int* d_current, int* d_next, int width, int height, int generation){
	int stride_x = gridDim.x * blockDim.x; //stride in x-direction
	int stride_y = gridDim.y * blockDim.y; //stride in y-direcion
    ...
    for(int i = x;i<width;i+=stride_x){
        for(int j = y;j<height;j+=stride_y){
            int index = j * width + i; //Cell selection 
            int LiveNeighbors = 0;  
    ...
    /*
            if (x == 0 && x == 0){ //prints output to console
                printf("Generation %d:\n", generation);
